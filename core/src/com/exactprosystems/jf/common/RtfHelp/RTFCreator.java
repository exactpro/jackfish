...
class RTFCreator {
    ...
    private void createContents() throws IOException
    {
        ...
        contents.add(p(text("2. "), text (" "), hyperlink(link + "Architecture", p("Architecture"))));
        contents.add(p(text("3. "), text (" "), hyperlink(link + "Requirements", p("System Requirements"))));
        contents.add(p(text("4. "), text (" "), hyperlink(link + "MVEL", p("MVEL"))));
        ...
        for( Map.Entry<String, List<RtfActionsHelper>> entry  : actionGroups.entrySet())
        {
            ...
            {
                innerCount++;
                contents.add(p(tab(), tab(), fontSize(fontSize, " 5." + count + "." + innerCount + " " ), fontSize(fontSize, " "), hyperlink(link + s.getName(), p(fontSize(fontSize, s.getName())))));
            }
            innerCount = 0;
        }
        ...
        {
            count++;
            contents.add(p(tab(), fontSize(fontSize, " 6." + count + " "), fontSize(fontSize, " "), hyperlink(link + name, p(fontSize(fontSize, name)))));
        }
        writeContents();
    }
            ...
            for (RtfActionsHelper rah : entry.getValue())
            {
                ...
                if (!classAnnotations.seeAlso().equals(""))
                {
                    ...
                    Arrays.asList(seeAlso).forEach(r-> {
                                String q = replaceChars(r);
                                seeAlsoText.add(hyperlink(link + q, pard(fontSize(fontSize, q))));
