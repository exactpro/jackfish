...
public class LibraryTreeNode extends TreeNode
{
	...
	public void display(List<String> librariesValue)
	{
		...
		Function<File, ContextMenu> menuFiles = file ->
		{
			...
			addNewLibrary.setOnAction(e -> Common.tryCatch(
					() -> ConfigurationTreeView.showInputDialog("Enter new name").ifPresent(
							name -> Common.tryCatch(() -> this.model.addNewLibrary(file, name), "Error on create new " + "library")),
		...
		librariesValue.forEach(file -> new BuildTree(new File(file), this.treeItem).doubleClickEvent(f -> () -> this.model.openLibrary(f))
				.fileFilter(f -> ConfigurationFx.getExtension(f.getAbsolutePath()).equals(ConfigurationFx.matrixExt)).menuTopFolder(menuTopFolder)
				.menuFiles(menuFiles).menuFolder(menuFolders).byPass());
	}
}