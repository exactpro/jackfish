...
namespace UIAdapter
{
    class Program
    {
        ...
        [DllExport("elementByCoords", CallingConvention.Cdecl)]
        public static int ElementByCoords([In, Out, MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 1)] int[] resultId, int length, int controlKindId, int x, int y)
        {
            try
            {
                var startMethod = getMilis();
                UpdateHandler();
                ControlKind controlKind = (ControlKind)controlKindId;
                var startUiFromPoint = getMilis();
                AutomationElement element = AutomationElement.FromPoint(new System.Windows.Point(x, y));
                logger.All("element from point", getMilis() - startUiFromPoint);
                ControlKind found = ByClassName(element.Current.ControlType);

                if (controlKind != ControlKind.Any && controlKind != found)
                {
                    bool flag = true;
                    while (flag)
                    {
                        var startFind = getMilis();
                        bool f = false;
                        ...
                        if (element != null)
                        {
                            found = ByClassName(element.Cached.ControlType);
                            try
                            {
                                FindByRuntimeId(element.GetRuntimeId());
                                f = true;
                            }
                            catch (Exception e)
                            {
                        ...
                        flag = !((element != null) && (controlKind == found) && f);
                        logger.All("Find from while by coor", getMilis() - startFind);
                    }
                }
                ...
                using (ElementHighLighter ehl = new ElementHighLighter(element))
                {
                    Thread.Sleep(500);
                }
                int[] res = element.GetRuntimeId();
                if (!cacheRuntimeId.ContainsKey(res))
                {
                    cacheRuntimeId.Add(res, element);
                }
                CopyArray(res, resultId, length);
                logger.All("method ElementsByCoords", getMilis() - startMethod);
                return res.Length;
            }
            catch (Exception e)
            {
